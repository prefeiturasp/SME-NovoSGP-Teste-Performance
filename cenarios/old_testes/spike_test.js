...
export function handleSummary(data) {
    return { "report/spike_test.html": htmlReport(data) };
}
...
export const options = {
    ...
    stages: [
        ...
        { duration: '5s', target: 1 }, // pico repentino
        { duration: '5s', target: 1 }, // volta ao normal
        { duration: '5s', target: 1 },
    ],
};
...
export default function () {
    ...
    const apiUrlIniciarProva = 'https://hom-serap-estudante.sme.prefeitura.sp.gov.br/api/v1/provas-tai/579/iniciar-prova';
    ...
    check(apiResponseIniciarProva, {
        'Verifica se o Status é 200 ou 411 no endpoint api/v1/provas-tai/325/iniciar-prova': (res) => res.status === 200 || res.status === 411,
    });
    ...
    const apiUrlProxima = 'https://hom-serap-estudante.sme.prefeitura.sp.gov.br/api/v1/provas-tai/579/proximo';
    ...
    check(apiResponseProxima, {
        'verifica se o Status é 200 endpoint api/v1/provas-tai/325/proximo': (res) => res.status === 200 || res.status === 411,
    });

    const apiUrlObterQuestao = 'https://hom-serap-estudante.sme.prefeitura.sp.gov.br/api/v1/provas-tai/579/obter-questao';
    ...
    check(apiResponseObterQuestao, {
        'Requisição autenticada obter questoes endpoint api/v1/provas-tai/579/obter-questao': (res) => res.status === 200 || res.status === 411,
    });
